...
def parse_pssm_to_normal_vector(pssm_filename):
    ...
    with open(pssm_filename, 'r') as file:
        for line in file:
            if flag.match(line):
                vector.append([float(x)/100 for x in line.split()[22:42]])
                if  len(line.split()[22:42]) != 20:
...
with open('./sieved_dataset_acc_pssm/trainset/trainset_remove_CN', 'r') as file:
    ...
    for N, LAM in zip(N_list, LAM_list):
        ...
        for i in [x for x in range(len(lines)) if x % 2 == 0]:
            ...
            # np.save('./sieved_dataset_acc_pssm/testset/right_pssm/vectors_lam12/{0}_lam12.npy'.format(protein_ID), np.array(vector))
            vector_list.append(vector)
        vector_np = np.array(vector_list)
        # label_np = np.array(label_list)
        # label_digit_np = np.array(label_digitalize(label_list))
        np.save('./sieved_dataset_acc_pssm/trainset/right_pssm/n{0}/vector_np_lam{1}.npy'.format(N, LAM), vector_np)
